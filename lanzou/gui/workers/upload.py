...
class UploadWorker(QThread):
    ...
    def _show_progress(self, file_name, total_size, now_size):
        ...
        self._task.rate = rate
        self.update.emit({self._url: self._task})
        ...
        if not self._is_work:
            ...
            while True:
                if not self._tasks:
                    ...
                    break
                self._url = list(self._tasks.keys())[0]
                self._task = self._tasks[self._url]
                logger.debug(f"run task: {self._task=}")
                if not os.path.exists(self._url):
                    logger.error(f"upload file not exist : {self._url}")
                    msg = f"<b>ERROR :</b> <font color='red'>文件不存在:{self._url}</font>"
                    self.code.emit(msg, 3100)
                    continue
                if os.path.isdir(self._url):
                    logger.error(f"upload dir : {self._url}")
                    msg = f"<b>INFO :</b> <font color='#00CC00'>批量上传文件夹:{self._url}</font>"
                    self.code.emit(msg, 30000)
                    self._disk.upload_dir(self._url,
                                          self._task.fid,
                                          self._show_progress,
                                          None,
                                          self._allow_big_file)
                else:
                    msg = f"<b>INFO :</b> <font color='#00CC00'>上传文件:{self._url}</font>"
                    self.code.emit(msg, 20000)
                    try:
                        code, fid, isfile = self._disk.upload_file(self._task.url,
                                                                   self._task.fid,
                                                                   self._show_progress,
                                                                   self._allow_big_file)
                    except TimeoutError:
